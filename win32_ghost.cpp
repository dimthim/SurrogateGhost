                           ...
                           HDC DeviceContext, int WindowWidth, int WindowHeight)
{
    ...
    real32 delta = .0001f;
    if((((real32)WindowWidth/(real32)WindowHeight) > ((real32)(Buffer->Width*2)/(real32)(Buffer->Height*2) - delta)) &&
           (((real32)WindowWidth/(real32)WindowHeight) < (((real32)Buffer->Width*2)/((real32)Buffer->Height*2) + delta)))
        {
...
/* NOTE: Joel Commented this out when he got rid of all replay buffers but 1*/
        ...
        switch(Message.message)
        {
            ...
            case WM_KEYUP:
            {
                ...
                if(WasDown != IsDown)
                {
                    ...
                    {
                        Win32ProcessKeyboardMessage(&KeyboardController->Load1, IsDown);
                    }
                    else if(VKCode == 0x32)
                    {
                        Win32ProcessKeyboardMessage(&KeyboardController->Load2, IsDown);
                    }
                    else if(VKCode == 0x33)
                    {
                        Win32ProcessKeyboardMessage(&KeyboardController->Load3, IsDown);
                    }
                    else if(VKCode == 0x34)
                    {
                        Win32ProcessKeyboardMessage(&KeyboardController->Load4, IsDown);
                    }
...
{    
    ...
    // Returns current amount of cycles. To convert to seconds, Do
    // Result / PerfCountFrequencyResult
        ...
        int ShowCode)
{
    ...
    // NOTE: LARGE_INTEGER is a Union that contains QuadPart as its member.
    // QuadPart is a 64-bit signed integer.
    // QueryPerfomanceFrequency() stores the cycles/second that your CPU is locked at
    // inside of the address that you pass to it.
    LARGE_INTEGER PerfCountFrequencyResult;
    QueryPerformanceFrequency(&PerfCountFrequencyResult);
    GlobalPerfCountFrequency = PerfCountFrequencyResult.QuadPart;

    // NOTE: Reads the string "win32_ghost.exe" into State->OnePastLastEXEFileNameSlash
    // and lops off all of the preceding "w:\\build\\"
    Win32GetEXEFileName(&Win32State);

    char SourceGameCodeDLLFullPath[WIN32_STATE_FILE_NAME_COUNT];
    Win32BuildEXEPathFileName(&Win32State, "ghost.dll",
                              sizeof(SourceGameCodeDLLFullPath), SourceGameCodeDLLFullPath);

    char TempGameCodeDLLFullPath[WIN32_STATE_FILE_NAME_COUNT];
    Win32BuildEXEPathFileName(&Win32State, "ghost_temp.dll",
    ...
    // NOTE(JOEL): According to msdn documentation, we should implement
    // timeEndPeriod(DesiredSchedulerMS() somewhere also. But this isn't part of
    // our production code so it probably doesn't matter that much.
    ...
    /* NOTE(casey): 1080p display mode is 1920x1080 -> Half of that is 960x540
       ...
       1024 + 128 = 1152
    */
    //IMPORTANT TODO NOTE: Start reworking DrawBitmap and DrawRectangle to Only draw to one side of the screen or another.
    //E.g. reserve the left half, or the left 2/3 of the screen for, or the middle, for the game and the side/sides for menus:
    //Inventory, clock, speech boxes, etc. This is really fun!
    //Also, make the screen 1520x860 or 1920x1080.
    //Also, look up the early handmade hero video where he left the above (casey) note to see his reasoning for what he wrote.
    Win32ResizeDIBSection(&GlobalBackbuffer, 1520, 860);
...
//    WindowClass.hIcon;
    WindowClass.lpszClassName = "GhostHeroWindowClass";
    ...
    if(RegisterClassA(&WindowClass))
    {
        ...
        if(Window)
        {
            ...
            // NOTE: This determines our the framerate we are trying to hit
            real32 GameUpdateHz = (real32)(MonitorRefreshHz / 1);
            real32 TargetSecondsPerFrame = 1.0f / (real32)GameUpdateHz;

            GlobalRunning = true;

#if 0
            //IMPORTANT(joel): I am commenting out the sound relevant code till I implement sound
            win32_sound_output SoundOutput = {};
            ...
            // On the Ghost Hero machine, it was 480 samples.
            while(GlobalRunning)
            {
                DWORD PlayCursor;
                DWORD WriteCursor;
...
/*TODO: Figure out how why the token/handle is invalid
            bool32 CanLargePage = true;  
...
*/
            ...
            /*NOTE(joel) commented this out 7/4
            ...
            */
            ...
            if(/*Samples &&*/ GameMemory.PermanentStorage && GameMemory.TransientStorage)
            {
                ...
                OldInput->dtForFrame = TargetSecondsPerFrame;


                //NOTE(joel): this appears to be the game loop
                while(GlobalRunning)
                {

                    /*
                    // NOTE: Here is some of the architecture that makes it possible
                    // for us to hotload coad.
...
#if 0 //IMPORTANT(joel): I am commenting out the sound relevant code till I implement sound
                        DWORD PlayCursor;
                        DWORD WriteCursor;
...
#endif //IMPORTANT(joel): I am commenting out the sound relevant code till I implement sound
#if 0 //IMPORTANT(joel): I am commenting out the sound relevant code till I implement sound
#if GHOST_INTERNAL
